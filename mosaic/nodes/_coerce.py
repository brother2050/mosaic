...
.. note::
   本模块与 :mod:`mosaic.nodes.image._image_utils` /
   :mod:`mosaic.nodes.video._video_utils` 中的同名函数职责不同：后两者在
   ``value`` 为 ``None`` 时直接抛异常，本模块则在提供 ``default`` 时回退
   ``default``。因此非 image / video 域节点应统一从本模块导入，避免跨域
   引用 image 域工具函数。
"""
