...
def example_2_prosody_control():
    ...
    # oral - 口语化连接词
    audio = tts.run(MosaicData(text="那个[oral_嗯]东西[oral_啊]真是太好用了")).get("audio")
    sf.write("output_chattts_oral.wav", audio.waveform, audio.sample_rate)
    print("[oral] 已保存")

    # laugh - 笑声
    audio = tts.run(MosaicData(text="哈哈哈，这个笑话太好笑了[laugh]")).get("audio")
    sf.write("output_chattts_laugh.wav", audio.waveform, audio.sample_rate)
    print("[laugh] 已保存")

    # break - 停顿
    audio = tts.run(MosaicData(text="第一句话[break]第二句话[break_500]第三句话")).get("audio")
    sf.write("output_chattts_break.wav", audio.waveform, audio.sample_rate)
    print("[break] 已保存")

    # speed - 局部语速
    audio = tts.run(MosaicData(text="[speed_0.8]这部分慢一点[ speed_1.5]这部分快一点")).get("audio")
    sf.write("output_chattts_speed.wav", audio.waveform, audio.sample_rate)
    print("[speed] 已保存")
...
def example_4_streaming():
    ...
    for chunk in tts.run_stream(MosaicData(text=text, language="zh")):
