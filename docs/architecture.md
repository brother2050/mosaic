...
```
文本 → SoVITSTokenizer → GPT2ARModel → FlowDecoder → SoVITSDecoder → StreamAdapter
       (切分+韵律)     (SSL特征)     (Flow)       (Flow+HiFiGAN)
...
mel frame: [f1] [f2] [f3] ... [fN]
           │    │    │       │
           ▼    ▼    ▼       ▼
flow decoder:  [chunk1: 1-30 frames]  [chunk2: 31-60 frames] ...
               │                       │
               ▼                       ▼
HiFiGAN:     [chunk1: ~1.3s]         [chunk2: ~1.3s] ...
             │                       │
             ▼                       ▼
audio:       ▓▓▓▓▓▓▓▓▓▓░░░░░░░░░▓▓▓▓▓▓▓▓▓▓▓▓░░░░░░
             ↑
             第一批延迟
             ~300ms（需要生成 ~30 frames）
```

**特点**：

- 非自回归，可一次性生成整段 mel
- 分块流式：每 N 帧（典型 30）通过声码器播放
