...
> 所有视频模型首次运行自动从 Hugging Face 下载。预下载命令：
> ```bash
> python -c "
...
### 1.1 Wan2.2 文生视频（MoE 架构）

```python
from mosaic.nodes.video import WanVideo
from mosaic import MosaicData

wan22 = WanVideo(model="Wan-AI/Wan2.2-T2V-A14B-Diffusers", dtype="bfloat16")

result = wan22.run(MosaicData(
    prompt="城市夜景中的霓虹灯雨，赛博朋克风格",
    num_frames=81,
    fps=16,
    num_inference_steps=30,
    guidance_scale=5.0,
))
result.get("video").save("cyber_rain.mp4")
```
