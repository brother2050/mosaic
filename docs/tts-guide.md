...
# 通过 speaker 参数指定音色
result = tts.run(MosaicData(text="第一句话", speaker="seed_222"))
audio1 = result.get("audio")

result = tts.run(MosaicData(text="第二句话", speaker="seed_222"))
audio2 = result.get("audio")

# 试听不同 speaker 找喜欢的声音
for spk in ["seed_2", "seed_222", "seed_786", "seed_2024"]:
    result = tts.run(MosaicData(text=f"这是{spk}号声音", speaker=spk))
    audio = result.get("audio")
```
