...
文本 "你好世界" + 参考音频 (1分钟)
  ...
  └─ phoneme_ids
  │
  ▼ GPT2ARModel (基于 SSL 特征自回归)
  └─ SSL 特征序列
  │
  ▼ FlowDecoder (SSL → mel)
  ▼ SoVITSDecoder = Flow + HiFiGAN
  │
  ▼ 32000Hz 单声道波形
```
...
audio = tts.run(text="Hello world, this is Fish Speech.", language="en")
audio.save("hello_en.wav")
```
...
audio = tts.run(
    text="CosyVoice 支持高质量中文合成。",
    language="zh",
    instruct="高兴地",  # 情感指令
)
audio.save("cosyvoice.wav")
```
...
audio = tts.run(text="无需 GPU 即可使用。", language="zh")
audio.save("cloud.wav")
```
...
**原理**：Flow Matching 是非自回归的，需要一次性 ODE 求解才能得到 mel 频谱。但为支持流式播放，CosyVoice 把 mel 频谱切成多个 chunk（默认 30 帧），每生成一个 chunk 就送入声码器播放。

```python
tts = TTS(backend="cosyvoice", streaming=True, chunk_size=30)

async for chunk in tts.synthesize_stream(
    text="长文本，CosyVoice 分块流式输出",
    language="zh",
):
    # 首批延迟约 300ms（需要生成前 30 帧 mel）
    play(chunk)
```
...
| 场景 | 推荐 chunk_size | 理由 |
|---|---|---|
| 实时对话 | 5-10 frames | 最低延迟 |
| 语音助手 | 15-20 frames | 平衡延迟和质量 |
| 长篇朗读 | 30 frames | 高吞吐 |
| 离线合成 | 完整 | 最高质量 |

```python
# 自定义 chunk_size（仅 CosyVoice 等分块流式后端生效）
tts = TTS(backend="cosyvoice", chunk_size=20)
```
...
ChatTTS 不支持真正的 zero-shot 克隆，但可以通过 `seed` 生成**稳定**的随机音色：

```python
tts = TTS(backend="chattts")

# 同一个 seed 总是生成同一个音色
audio1 = tts.run(text="第一句话", seed=42)
audio2 = tts.run(text="第二句话", seed=42)

# 试听不同 seed 找喜欢的声音
for seed in [42, 123, 999, 2024]:
    audio = tts.run(text="这是第 {} 号声音".format(seed), seed=seed)
    audio.save(f"voice_{seed}.wav")
```
...
```python
tts = TTS(backend="fish", ref_audio="reference.wav", ref_text="参考音频的文字内容")

# 任意新文本都会用参考音频的音色
audio = tts.run(text="新的文本内容", language="zh")
...
```python
tts = TTS(backend="sovits", ref_audio="short_ref.wav")

audio = tts.run(
    text="这是用 5 秒参考音频克隆的声音",
    language="zh",
)
audio.save("sovits_clone.wav")
```
...
```python
tts = TTS(backend="sovits", ref_audio="my_speaker.wav")
```
...
```python
tts = TTS(backend="cosyvoice", ref_audio="ref.wav", ref_text="参考文本")
...
# break - 停顿
audio = tts.run(
    text="第一句[break]第二句[break_500]第三句",
    language="zh",
)

# speed - 语速控制（按段）
audio = tts.run(
    text="[speed_0.8]慢一点[ speed_1.2]快一点",
    language="zh",
)
```
...
```python
# ChatTTS 自动处理
audio = tts.run(text="我的电话号码是 138-0013-8000", language="zh")
...
# ChatTTS
tts = TTS(backend="chattts")
zh_audio = tts.run(text="你好世界", language="zh")
en_audio = tts.run(text="Hello world", language="en")
ja_audio = tts.run(text="こんにちは", language="ja")

# Fish Speech（多语言最佳）
tts = TTS(backend="fish")
for lang, text in [("zh", "你好"), ("en", "Hello"), ("ja", "こんにちは"), ("ko", "안녕하세요")]:
    audio = tts.run(text=text, language=lang)
    audio.save(f"hello_{lang}.wav")

# CosyVoice
tts = TTS(backend="cosyvoice")
audio = tts.run(text="Hello world in English", language="en")
```
...
```python
tts = TTS(backend="cosyvoice", dtype="bfloat16")  # 减半显存
```
...
# 后续使用
tts = TTS(backend="sovits", speaker_features=features)
```
